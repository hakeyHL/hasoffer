...
public class Image2DownloadJobBean extends QuartzJobBean {

    private static final Logger logger = LoggerFactory.getLogger(Image2DownloadJobBean.class);

    /**
     * 取ptmimage 逻辑：未下载下来的图片，按照失败次数从小到大排
     */
    private static final String Q_PTM_IMAGE =
            "SELECT t FROM PtmImage t WHERE t.path2='' and t.imageUrl2<>'' ORDER BY t.errTimes ASC,t.id ASC";
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        SysTimerTaskLog log = timerService.createTaskLog("Image2DownloadTask");
        ...
        while (page <= totalPage) {
            ...
            if (ArrayUtils.hasObjs(images)) {
                for (PtmImage image : images) {
                    imageService.downloadImage2(image);
                }
            }
