...
public class SpiderConfigServiceImpl implements ISpiderConfigService {
    ...
    public SpiderConfig findById(long id) {
        String sql = "select id, website,thread_num, time_out,  retry_times, interval_times,task_level from spider_config where id=?";
        Connection conn = JdbcUtil.getConnection();
        SpiderConfig result = null;
        if (null != conn) {
            ...
            try {
                ...
                while (rs.next() && rs.isFirst()) {
                    ...
                    result = new SpiderConfig(websiteTemp, threadNum, timeOut, retryTimes, intervalTimes, taskLevel);
                    result.setId(rId);
                }
    ...
    @Override
    public SpiderConfig findByWebsite(Website website) {
        if (null != website) {
            SpiderConfig spiderConfig = null;
            String sql = "select id, website,thread_num, time_out,  retry_times, interval_times,task_level from spider_config where website=?";
            Connection conn = JdbcUtil.getConnection();
            if (null != conn) {
                PreparedStatement stmt = null;
                ResultSet rs = null;
