        ...
        while (true) {
            ...
            try {
                if (searchLogQueue.size() > 800) {
                    ...
                    logger.debug("UnmatchedSearchRecordListWorker . list job has no jobs. go to sleep!");
                    continue;
                }

                PageableResult<SrmSearchLog> pagedSearchLog = searchService.listNoresultSearchLogs(startDate, 1, 500);
...
//                    startDate = searchLogs.get(searchLogs.size() - 1).getCreateTime();
                    for (SrmSearchLog log : searchLogs) {
                        startDate = log.getCreateTime();
                        searchLogQueue.add(log);
                    }
                }else{
