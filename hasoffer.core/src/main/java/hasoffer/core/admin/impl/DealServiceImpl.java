...
//    private static final String IMPORT_SQL = "insert into appdeal(website, title, linkUrl, expireTime, priceDescription ,description, createTime,  push ,display ,imageUrl,discount,dealCategoryId,dealClickCount,appdealSource,ptmcmpskuid) values(?,?, ?, ?, ?, ? ,?, ?, ?, ?,?,?,?,'MANUAL_INPUT',0)";
    ...
    @Override
    public List<AppDeal> getValDealByLinkUrl(String url) {
        return dbm.query("SELECT t FROM AppDeal t WHERE t.linkUrl = ?0 and now()<t.expireTime", Arrays.asList(url));
    }

    @Override
    public AppDeal getDealBySourceId(Long skuId) {
        List<AppDeal> appdealList = dbm.query("SELECT t FROM AppDeal t WHERE t.appdealSource = 'PRICE_OFF' AND t.ptmcmpskuid = ?0 ORDER BY t.expireTime DESC", Arrays.asList(skuId));
        if (appdealList == null || appdealList.size() == 0) {
            return null;
        } else {
            return appdealList.get(0);
        }
    }
