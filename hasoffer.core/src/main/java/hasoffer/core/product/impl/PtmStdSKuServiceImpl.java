...
public class PtmStdSKuServiceImpl implements IPtmStdSkuService {
    ...
    public void importPtmStdSku2Solr(PtmStdSku ptmStdSku) {
        ...
        if (ptmStdSku1 == null) {
            ...
            ptmStdSkuIndexServicel.remove(ptmStdSku.getId() + ConstantUtil.API_DATA_EMPTYSTRING);
            return;
        }
...
//        ptmStdSkuModel.setSearchCount(searchCount == null ? 0 : searchCount.getCount());
    ...
    private boolean compareIgnoreCase(String name, CategoryFilterParams params) {
        name = name.toLowerCase().replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING);
        String paramString = params.name().toLowerCase();
        if (paramString.contains("_")) {
            paramString = paramString.replaceAll("_", ConstantUtil.API_DATA_EMPTYSTRING);
        }
...
//        try {
    ...
    private void setOpeartingSystem(String opreatingSystem, PtmStdSkuModel ptmStdSkuModel) {
        if (StringUtils.isNotEmpty(opreatingSystem)) {
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Android".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Android");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Bada".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Bada");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Blackberry".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Blackberry");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Blackberry OS".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Blackberry OS");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Brew".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Brew");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Firefox".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Firefox");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("iOS".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("iOS");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Linux".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Linux");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Nokia".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Nokia");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Asha".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Asha");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Nokia X Software".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Nokia X Software");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Propreitory".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Propreitory");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Sailfish".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Sailfish");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Symbian".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Symbian");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Tizen".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
                ptmStdSkuModel.setOperating_System("Tizen");
                return;
            }
            if (opreatingSystem.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("Windows".replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase())) {
    ...
    public void setGeneral(PtmStdSkuModel ptmStdSkuModel, PtmStdSkuParamNode ptmStdSkuParamNode, String name, StringBuilder hakeySb) {
        ...
        if (compareIgnoreCase(name, CategoryFilterParams.Brand)) {
            ...
            if (priceBelowScope != null && ptmStdSkuModel.getCate2() == 5l) {
                ...
                if (ptmStdSkuParamNode.getValue().toLowerCase().contains("mobiles")) {
                    ptmStdSkuParamNode.setValue(ptmStdSkuParamNode.getValue().replaceAll("mobiles", " ").replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING));
                }
        ...
        if (compareIgnoreCase(name, CategoryFilterParams.Launch_Date)) {
            ...
            if (launchDateString.contains(" (Official)")) {
                launchDateString = launchDateString.replace(" (Official)", ConstantUtil.API_DATA_EMPTYSTRING);
            }
            try {
    ...
    public void setDisplays(PtmStdSkuModel ptmStdSkuModel, PtmStdSkuParamNode ptmStdSkuParamNode, String name) {
        ...
        if (compareIgnoreCase(name, CategoryFilterParams.Screen_Size)) {
            ...
            if (inch != -1) {
                screenSize = screenSize.substring(0, inch).replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING);
        ...
        if (compareIgnoreCase(name, CategoryFilterParams.Screen_Resolution)) {
            ...
            if (StringUtils.isNotEmpty(screenResolution)) {
                ...
                if (screenResolution.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("1920x1080") || screenResolution.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("1080x1920")) {
                    ptmStdSkuModel.setScreen_Resolution("1920x1080 (Full HD)");
                } else if (screenResolution.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("1280x720") || screenResolution.replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING).toLowerCase().contains("720x1280")) {
        ...
        if (compareIgnoreCase(name, CategoryFilterParams.Screen_to_Body_Ratio)) {
            ...
            if (StringUtils.isNotEmpty(value) && value.contains("%")) {
                value = value.substring(0, value.indexOf("%")).replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING);
    ...
    public void setPerformance(PtmStdSkuModel ptmStdSkuModel, PtmStdSkuParamNode ptmStdSkuParamNode, String name) {
        ...
        if (compareIgnoreCase(name, CategoryFilterParams.RAM)) {
            ...
            if (numberFromString != -1) {
                ...
                if (!stringRam.equals(ConstantUtil.API_DATA_EMPTYSTRING)) {
                    if (ram.contains("GB")) {
    ...
    public void setMainCamera(PtmStdSkuModel ptmStdSkuModel, PtmStdSkuParamNode ptmStdSkuParamNode, String name) {
        ...
        if (compareIgnoreCase(name, CategoryFilterParams.RESOLUTION)) {
            ...
            if (mp != -1) {
                float floatResolution = Float.parseFloat(resolution.substring(0, mp).replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING));
    ...
    public void setBattery(PtmStdSkuModel ptmStdSkuModel, PtmStdSkuParamNode ptmStdSkuParamNode, String name) {
        ...
        if (compareIgnoreCase(name, CategoryFilterParams.CAPACITY)) {
            ...
            if (batteryCapacity.toLowerCase().contains("mah")) {
                ...
                if (mAh != -1) {
                    batteryCapacity = batteryCapacity.substring(0, mAh).replaceAll(" ", ConstantUtil.API_DATA_EMPTYSTRING);
