    ...
    public String getPriceCurve(@RequestParam(defaultValue = "0") Long id, HttpServletResponse response) {
        ...
        if (priceNodes != null && priceNodes.size() != 0) {
            ...
            if (flag) {
                ...
                float price = priceNodes.get(0).getPrice();
                tempPriceNodes.add(priceNodes.get(0));
                for (PriceNode priceNode : priceNodes) {
                    if (price != priceNode.getPrice()) {
                        tempPriceNodes.add(priceNode);
                    }
                }
                if (tempPriceNodes.size() == 1) {
                    //只有一种价格
                    ...
                    //3.4 a+(b-a)/2
                    BigDecimal pointTwo = (BigDecimal.valueOf(maxPrice).add(BigDecimal.valueOf(minPrice))).divide(BigDecimal.valueOf(2), BigDecimal.ROUND_HALF_UP);
//                    BigDecimal pointTwo = a.add((b.subtract(a)).divide(BigDecimal.valueOf(2)));
...
//                    Y.add(pointThree.longValue());
                    ...
                    //如果有大于1个数据则代表其有价格变化
                    //1.1 按照日期剩升序给出
                    Collections.sort(tempPriceNodes, new Comparator<PriceNode>() {
                        @Override
                        public int compare(PriceNode o1, PriceNode o2) {
                            if (o1.getPriceTimeL() < o2.getPriceTimeL()) {
                                return -1;
                    ...
                    //1.2 过滤不合法数据和添加辅助点
                    Iterator<PriceNode> iterator = tempPriceNodes.iterator();
                    while (iterator.hasNext()) {
                        PriceNode priceNode = iterator.next();
                        if (priceNode.getPrice() <= 0) {
                            iterator.remove();
                    ...
                    LinkedList<PriceNode> lPriceNodes = new LinkedList();
                    if (!getDateMMdd(priceNodes.get(priceNodes.size() - 1).getPriceTimeL()).equals(getDateMMdd(new Date().getTime()))) {
                        tempPriceNodes.add(new PriceNode(new Date(), priceNodes.get(priceNodes.size() - 1).getPrice()));
                    }
                    int priceNodesSize = tempPriceNodes.size();
                    int temp = 0;
                    lPriceNodes.add(new PriceNode(tempPriceNodes.get(0).getPriceTime(), tempPriceNodes.get(0).getPrice()));

                    for (int j = 1; j < priceNodesSize; j++) {
                        PriceNode priceNo = tempPriceNodes.get(i);
                        System.out.println("array " + temp + "  is  " + getDateMMdd(priceNo.getPriceTimeL()) + "  and price is :" + priceNo.getPrice());
                        //除了第一个,如果当前的前一天与上一个值不相同则增加前一天这个点
                        long priorDateLong = priceNo.getPriceTimeL() - 1000 * 60 * 60 * 24;
                        String priorDate = getDateMMdd(priorDateLong);
                        System.out.println(" priorDate " + priorDate);
                        if (!priorDate.equals(getDateMMdd(tempPriceNodes.get(i - 1).getPriceTimeL()))) {
                            System.out.println("not equal ");
                            Date date = new Date();
                            date.setTime(priorDateLong);
                            System.out.println("add node :  " + priorDate + " price " + priceNodes.get(i - 1).getPrice());
                    ...
                    tempPriceNodes = null;
                    System.gc();
                    tempPriceNodes = new ArrayList<>();
                    tempPriceNodes.addAll(lPriceNodes);
                    System.out.println(" tempPriceNodes " + tempPriceNodes.size());
                    //5. 给出坐标集合
                    if (tempPriceNodes != null && tempPriceNodes.size() > 0) {
                        for (PriceNode priceNode : tempPriceNodes) {
                            System.out.println(" Time :" + getDateMMdd(priceNode.getPriceTimeL()) + " price :" + priceNode.getPrice());
                            //查询到价格历史,开始分析priceTimeL
                            PriceCurveXYVo priceCurveXYVo = new PriceCurveXYVo(this.getDateMMdd(priceNode.getPriceTimeL()), BigDecimal.valueOf(priceNode.getPrice()).longValue(), getDistance2X(priceTimeL, priceNode.getPriceTimeL()));
                            priceXY.add(priceCurveXYVo);
