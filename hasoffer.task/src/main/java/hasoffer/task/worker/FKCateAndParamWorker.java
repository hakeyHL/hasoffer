...
public class FKCateAndParamWorker implements Runnable {
    ...
    private final String Q_CATEGORY_BYNAME = "SELECT t FROM PtmCategory3 t WHERE t.name = ?0 ";
    ...
    public FKCateAndParamWorker(IDataBaseManager dbm, ListAndProcessWorkerStatus<PtmCmpSku> ws, ICategoryService categoryService) {//, IMongoDbManager mdm
        this.dbm = dbm;
//        this.mdm = mdm;
        this.ws = ws;
...
//        final String DESCRIPTION_INFO2 = "//div[@id='veiwMoreSpecifications']/table";
        ...
        //获取导航栏中catePath的长度，取前3位或者更小
        int cateSize = catePathList.size();
        cateSize = cateSize > 4 ? 4 : cateSize;
...
//        }
        ...
        for (int i = 0; i < cateSize; i++) {
            ...
            try {
                ...
                //检查类目是否存在
                PtmCategory3 category = dbm.querySingle(Q_CATEGORY_BYNAME, Arrays.asList(pathString));

                if (category != null) {
                    parentId = category.getId();
                } else {
                    category = new PtmCategory3();
        ...
        updater.getPo().setCategoryId2(parentId);
        dbm.update(updater);
